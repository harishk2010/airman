  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      <div className="cockpit-card overflow-hidden">
        {loading ? (
          <div className="p-6 space-y-3">{[1,2,3,4,5].map(i => <div key={i} className="h-12 shimmer rounded-lg" />)}</div>
        ) : logs.length === 0 ? (
                ...
                {logs.map(log => (
                  <>
                    <tr key={log.id} className="cursor-pointer" onClick={() => setExpanded(expanded === log.id ? null : log.id)}>
                      <td>
                        <div className="font-mono text-xs">{formatDateTime(log.created_at)}</div>
                        <div className="font-mono text-[10px] text-sky-400/30">{timeAgo(log.created_at)}</div>
                      </td>
                      <td>
                      ...
                      <td>
                        <div className="text-xs text-sky-300/70">{log.entity_type}</div>
                        {log.entity_id && <div className="font-mono text-[10px] text-sky-400/30">{log.entity_id.slice(0,8)}...</div>}
                      </td>
                      <td>
                      ...
                      <tr key={`${log.id}-expanded`}>
                        <td colSpan={5} className="bg-sky-950/30 !py-4 !px-6">
                          ...
                          <div className="flex gap-6 mt-2 text-[10px] font-mono text-sky-400/30">
                            ...
                            {log.correlation_id && <span>CID: {log.correlation_id}</span>}
                          </div>
                        </td>
                      </tr>
                    )}
        ...
        {total > 30 && (
          <div className="flex items-center justify-center gap-2 p-4 border-t border-white/[0.05]">
            <button className="btn-secondary px-4 py-2 text-xs" onClick={() => setPage(p => Math.max(1, p-1))} disabled={page === 1}>← Prev</button>
            <span className="font-mono text-xs text-sky-400/50 px-3">{page} / {Math.ceil(total/30)}</span>
            <button className="btn-secondary px-4 py-2 text-xs" onClick={() => setPage(p => p+1)} disabled={page >= Math.ceil(total/30)}>Next →</button>
          </div>
        )}
      </div>
    </div>
